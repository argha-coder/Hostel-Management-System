...
const Login = () => {
  ...
  const handleLoginSubmit = async (e) => {
    ...
    if (!email.endsWith('@gmail.com') && !email.endsWith('@example.com') && !email.endsWith('@uhostel.com')) {
      setError('Only @gmail.com, @example.com or @uhostel.com addresses are allowed'); return;
    }
        ...
        <div style={{ width: '100%', maxWidth: '420px', position: 'relative', zIndex: 1 }}>
          ...
          <AnimatePresence mode="wait">
            {step === 1 ? (
              <motion.form key="step1" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }}
                onSubmit={handleLoginSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}
              >
                {/* Email */}
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    ...
                    <Link to="#" style={{ fontSize: '0.8rem', color: '#6366f1', textDecoration: 'none', fontWeight: 600 }}>Forgot password?</Link>
                  </div>
                  <div style={{ position: 'relative' }}>
          ...
          >
            <p style={{ color: '#64748B', fontSize: '0.9rem' }}>
              ...
              <Link to="/register" style={{ color: '#6366f1', textDecoration: 'none', fontWeight: 700 }}>Sign up</Link>
            </p>
          </motion.div>
        </div>
      </motion.div>
